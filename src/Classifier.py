...
def load_checkpoint(filepath, lr=0.001, dropout=0.2, device="cpu"):
    ...
    model, optimizer, criterion = defineModel(checkpoint['input_size'], checkpoint['output_size'], checkpoint['hidden_layers'], dropout=dropout, lr=lr, device=device)
...
def defineModel(input, output, hidden_layers, dropout=0.2, lr=0.001, device="cpu"):
    model = Classifier(input_size=input, output_size=output, hidden_layers=hidden_layers, drop_p=dropout)
    model.to(device)
