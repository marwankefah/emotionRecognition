...
def loadData(path, transforms=[], batchsize=4096):
    ...
    trainloader = torch.utils.data.DataLoader(data, batch_size=batchsize, shuffle=True)
    return trainloader
...
def loadsplitData(path, transforms, train_batchsize=10000, test_batchsize=4000):
    ...
    trainloader = torch.utils.data.DataLoader(data_train, batch_size=train_batchsize, shuffle=True)
    testloader = torch.utils.data.DataLoader(data_test, batch_size=test_batchsize)
...
def getProcessedData(path1, path2, train_batchsize=512, test_batchsize=512, transform=defineTransforms(), device="cpu"):
    ...
    trainloader, testloader = loadsplitData(path1, transforms=transform, train_batchsize=14000, test_batchsize=2500)
    real, imag = build_filters()
...
def getmodelfortesting(path, lr=0.001, dropout=0.2, device="cpu"):
    model, optimizer, criterion = load_checkpoint(path, lr=lr, dropout=dropout, device=device)
    return model
...
def getemotion(img, path1, path2):
    ...
    img = processimage(img=img, transforms=transforms, path=path2, device=device)
