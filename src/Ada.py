...
class AdaBoostSelection:
    ...
    def fit(self, X, Y):
        ...
        for i in range(self.T):
            ...
            threshpos = np.sum(w[Y == 1] * X[Y == 1], axis=0) / np.sum(w[Y == 1])
            threshneg = np.sum(w[Y == 0] * X[Y == 0], axis=0) / np.sum(w[Y == 0])
            threshold = 0.5 * (threshpos + threshneg)
            p = np.ones((X.shape[1]))
            p[threshneg < threshpos] = -1
            # Parity for comparison
            h = p * X <= p * threshold
            error = np.sum(w * np.abs(h - Y.reshape(Y.shape[0], 1)), axis=0) / np.sum(w)
            error[self.ChosenFeatures == 1] = 1

            chosen = np.argsort(error)  # sorting errors in ascending order and getting the indices sorted
